...
// コンストラクタ: CANManagerへの登録処理を追加
LKMotor::LKMotor(CAN &can, CANManager &can_manager, uint8_t motor_count)
...
void LKMotor::sendCommand(uint8_t motorID, uint8_t cmd, const uint8_t data[8]) { // モーターへの指示送信関数
    ...
    for (int i = 1; i < 8; i++) {
        frame[i] = data[i]; // 送信データの1~7番目に送信データの情報を書き込む
    }
...
// --- CANManager から呼び出される受信ハンドラ ---
bool LKMotor::handle_message(const CANMessage &msg) {
    // このクラスが管理するモーターのID範囲内かチェック
    if (msg.id >= 0x141 && msg.id <= 0x140 + _motorCount) {
        uint8_t i = (msg.id - 0x141); // 配列のインデックス (0 ~ motorCount-1)
        ...
        return true; // 処理対象のIDだったので、これ以上他のReceiverに回さないようtrueを返す
    }
    return false; // 自分に関係のないIDだった
}
...
void LKMotor::trq_control(int motor_index, int16_t torqueControl) {
    ...
    uint8_t data[8] = {0xA1, 0, 0, 0,
        ...
        0, 0};
    if (!state_flag) sendCommand(motor_index + 1, 0xA1, data);
}
...
void LKMotor::inc_angle_controlAll(int32_t* angleIncrements) {
    for (int i = 0; i < _motorCount; i++) inc_angle_control(i, angleIncrements[i]); // 's' を追加
}
...
LKMState LKMotor::read(uint8_t index) const {
    if (index >= _motorCount) return {0};
