#ifndef INCLUDED_LKMOTOR_H
#define INCLUDED_LKMOTOR_H
#pragma once
...
// CANReceiver インターフェースを継承
class LKMotor : public CANReceiver {
public:
    // 送信用に CAN、受信登録用に CANManager の両方を受け取る
    LKMotor(CAN &can, CANManager &can_manager, uint8_t motor_count);
    
    // CANReceiver の純粋仮想関数を実装
    // 自分に関係するIDなら処理して true を返す
